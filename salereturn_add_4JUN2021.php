...
if(isset($_POST['save']))
{
	...
	$NumberCheckQ="SELECT MAX(sr_NumberSr) as sr_Number FROM cust_salereturn WHERE 1 AND branch_id='$branch_id' ";
	...
	if(mysqli_query($con,$sQ))
	{
		...
		foreach ($item_idArray as $key => $item_id)
		{
			...
			$item_SaleScheme=0;
			$item_DiscountPercentage=0;
			$item_DiscountPrice=0;
			$item_SaleExtraAmount=0;
			$item_NetPrice=$item_NetPriceArray[$key];
			
	 		$sdQ="INSERT INTO cust_salereturn_detail (sr_id, srd_Date, item_id, item_BarCode,		item_IMEI, item_Qty,		item_SalePrice,		item_CostPrice,		item_NetPrice,srd_CreatedOn,client_id)
										VALUES ($s_id,'$s_Date',$item_id,'$item_BarCode',	'$item_IMEI', '$item_Qty',	'$item_SalePrice',	'$item_CostPrice',	'$item_NetPrice',now(),$client_id)";
	...
	if(!empty($_POST['sp_Amount']))
	{
		...
		$spQ="INSERT INTO adm_sale_payment(client_id, sp_Amount, sp_Date, s_id, sp_Description,sp_Type, sp_CreatedOn, u_id, branch_id) VALUES ($client_id,'$sp_Amount','$s_Date',$s_id,'Sale Return Payment','SR',now(), '$u_id', '$branch_id')";
...
<form  class="smart-form" novalidate="novalidate" method="post" action="" onsubmit="return checkParametersInvoice();">
<table border="0" style="width:50%; float:left; margin-top:20px;" class="">
    <tr>
        <td style="text-align: center;line-height: 36px;font-weight: bold;">Enter Invoice Number</td>
        <td><input type="text" name="post_invoiceno" id="post_invoiceno" class="form-control" value="<?=isset($_POST['post_invoiceno']) ? $_POST['post_invoiceno'] : ''?>" placeholder="Enter Invoice No." required style="font-weight:bold;" /></td>
        <td><input type="submit" name="post_invoice" class="btn btn-primary btn-sm" /></td>
    </tr>
</table>    
</form>
...
<?php
$show_form=0;
if(isset($_POST['post_invoiceno'],$_POST['post_invoice']))
{
	$s_Number=validate_input($_POST['post_invoiceno']);
	if(empty($s_Number))
	{
		$msg="Invoice Number should not be empty";
	}
	else
	{
		$query="SELECT  S.s_Date, S.s_CreatedOn, S.s_Number, S.s_id, S.client_id, S.s_TotalAmount, S.s_DiscountAmount, S.s_NetAmount, S.s_PaidAmount, S.s_SaleMode,S.s_TaxAmount, S.s_Remarks, S.s_RemarksExternal, C.client_Name,SD.item_id, SD.item_Qty, SD.item_BarCode, SD.item_IMEI, SD.item_SalePrice,SD.item_DiscountPercentage,SD.item_DiscountPrice,SD.item_NetPrice,I.item_Name,I.item_Code,B.brand_Name
				FROM cust_sale AS S
				...
				WHERE S.s_Number='$s_Number' AND S.branch_id=$branch_id
...
{
?>
<table style="width:75%; float:left; margin-top:20px;border: 6px solid #f1f1f1 !important; margin-left: 10px;" class="table table-bordered">
	<tr><th style="text-align:center; background:#009dff; color:#fff;">Customer Name</th><td colspan="3"><?=$saleArray[0]['client_Name']?></td><td colspan="2" style="text-align:center;"><?=date('d-m-Y h:i:s', strtotime($saleArray[0]['s_CreatedOn']))?></td></tr>
    <tr style="background:#009dff; color:#fff;">
    ...
    <?php
	foreach($saleArray as $key => $row)
	{
		?>
        	<tr>
                <td style="text-align:center;"><?=$row['item_Code']?></td>
                <td style="text-align:center;"><?=$row['item_Name']?></td>
                <td style="text-align:center;"><?=$row['item_IMEI']?></td>
                <td style="text-align:center;"><?=$row['item_Qty']?></td>
                <td style="text-align:center;"><?=$row['item_SalePrice']?></td>
                <td style="text-align:center;"><?=$row['item_NetPrice']?></td>
            </tr>
        <?php
	}
	?>
...
<form id="checkout-form" class="smart-form" novalidate="novalidate" method="post" action="" onsubmit="return checkParameters();">	
    ...
    <input type="hidden" value="<?=$saleArray[0]['s_id']?>" name="s_id" id="s_id" />
    <input type="hidden" value="<?=$saleArray[0]['client_id']?>" name="client_id" id="client_id" />
                    ...
                    <td>
                    	<input list="item_list"  name="" id="ex_item" class="form-control" placeholder="Search Product by Name" required autocomplete="off"  onblur="getItemDetail()" style="padding-left:8px;">
                            <datalist id="item_list">
                                ...
                                <?php
								 foreach ($saleArray as $key => $itemRow) { 
								?>
								<option value="<?php echo $itemRow['item_Name'];?>" data-item-id="<?php echo $itemRow['item_id'];?>" data-item-qty="<?php echo $itemRow['item_Qty'];?>" data-item-saleprice="<?php echo $itemRow['item_SalePrice'];?>" data-item-netprice="<?php echo $itemRow['item_NetPrice'];?>" data-item-name="<?php echo $itemRow['item_Name'];?>">
								<?php } ?>
			...
			<div style="min-height:100px;overflow:auto"><!--A wrapper div to control the height of table-->
                ...
                <div class="" style="margin-top:10px">
                    <div class="col-lg-12 col-md-12 col-xs-12">
                        <table style="width:100%; background:#f1f1f1; display:none;" class="table table-condensed">
                            <tr>
                                ...
                                <th style="width:50%" rowspan="2">
                                	<table style="width:100%;">
                                    	<tr>
                                        	<td>Discount(%)<br /><input type="number" name="s_Discount" id="s_Discount" placeholder="Enter Diss. %" class="form-control" onkeyup="calculate()"  onchange="calculate();" style="text-align:right"  min="0"></td>
                                        	<td>&nbsp;</td>
                                            <td>VAT(%)<br /><input type="number" name="s_Tax"  placeholder="Enter Tax %" id="s_Tax" class="form-control" style="text-align:right" min="0" onkeyup="calculate()" onchange="calculate();"></td>
                                            <td>&nbsp;</td>
                ...
                <table style="background:#09F; padding:10px; width:40%; float:right;">
                	<tr>
...
<?php } //end of bracket show form if item exists against sale invoice ?>
</div>
...
<!-- function to check low sale price then purchse price -->
