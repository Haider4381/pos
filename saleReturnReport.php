...
<table style="width:100%;" align="center" border="0">
	...
	<tr><td style="text-align:center;"><h3><?php echo $branchRow['branch_Name'];?></h3><?php echo $branchRow['branch_Address'];?><br /><?php echo $branchRow['branch_Phone1'];?></td></tr>
						 ...
						 <br></div>				
						 <div style=" margin-left:0%; text-align:center;"><b style="font-size:20px;">Sale Return Report</b><br/></div>
...
?>	
					...
					{
						$from_date=date('Y-m-d',strtotime($_POST['from_date']));
						$where.=" AND sr.sr_Date>='$from_date'";
					}
					if(!empty($_POST['to_date']))
					{
						$to_date=date('Y-m-d',strtotime($_POST['to_date']));
						$where.=" AND sr.sr_Date<='$to_date'";
					}
					if($_POST['item_id']!='ALL')
					{
						$item_id=$_POST['item_id'];
						$where.=" AND srd.item_id=$item_id";
					}
					if($_POST['client_id']!='ALL')
					{
						$client_id=$_POST['client_id'];
						$where.=" AND sr.client_id=$client_id";
					}
					
					
					
					$query ="SELECT
	sr.sr_Date as sale_Date, srd.item_id,  (srd.item_Qty) as item_Qty,  (srd.item_SalePrice) as item_SalePrice, item_NetPrice as item_TotalAmt,
	i.item_Name, icat_name, isubcat_name, client_Name
FROM cust_salereturn as sr
INNER JOIN cust_salereturn_detail as srd ON sr.sr_id=srd.sr_id
INNER JOIN adm_item as i ON srd.item_id=i.item_id
INNER JOIN adm_client on adm_client.client_id=sr.client_id
...
LEFT OUTER JOIN adm_itemsubcategory as subcat ON i.isubcat_id=subcat.isubcat_id
						$where AND sr.branch_id=$branch_id
						ORDER BY sr.sr_Date ASC, icat_name ASC,isubcat_name, i.item_Name ASC";
					//echo '<pre>'.$query.'</pre>';
					$results = mysqli_query($con,$query);
					if(mysqli_num_rows($results)<1)
					{
