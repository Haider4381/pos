...
$pQ = "SELECT S.sr_id, S.sr_Date, S.sr_Number, S.client_id, S.sr_TotalAmount, S.sr_DiscountPrice, S.sr_NetAmount, S.sr_Remarks, COUNT(SD.srd_id) AS TotalItems, AC.account_title AS client_Name, sale.s_Number
    FROM cust_salereturn AS S
    LEFT JOIN cust_salereturn_detail  AS SD ON SD.sr_id = S.sr_id
    LEFT OUTER JOIN cust_sale AS sale ON sale.s_id = S.s_id 
    LEFT JOIN accounts_chart AS AC ON AC.account_id = S.client_id
			...
			<table id="datatable_fixed_column" class="table table-striped table-bordered" width="100%">
		        ...
		        <thead>
					<tr>
                        ...
                        <th class="hasinput" style="width:10%">
							<input type="text" class="form-control" placeholder="Sale Return No." />
						</th>
		            ...
		            <tr>
                        ...
                        <th style="text-align:center;">Sale Return No.</th>
		        ...
		        foreach ($row as $key => $r) 
		        {
		     	?>
		            <tr id="row<?=$r['sr_id']?>">
                        ...
                        <td style="text-align:center;"><?php echo $r['sr_Number']; ?></td>
                        <td style="text-align:center;"><?php echo sum_date_formate($r['sr_Date']); ?></td>
		                <td style="text-align:center;"><?php echo $r['client_Name']; ?></td>
		                <td style="text-align:center;"><?php echo $r['sr_NetAmount']; ?></td>
		                <td style="text-align:center;"><?php echo $r['TotalItems']; ?></td>
		                <td style="text-align:center;">
		                	<a href="salereturn_print.php?sr_id=<?=$r['sr_id'];?>" target="_blank" class="btn btn-success btn-sm">Print</a>
		                	<a href="salereturn_list_detail.php?sr_id=<?=$r['sr_id'];?>" target="_blank" class="btn btn-primary btn-sm">View Detail</a>

                            <a href="#" onclick="del(<?=$r['sr_id'];?>)" class="btn btn-danger btn-sm">Delete</a>
		                </td>
		            </tr>
		        <?php
		        }
