...
<div class="row">
	...
	<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		...
		<div class="jarviswidget jarviswidget-color-purity" id="wid-id-1" data-widget-editbutton="false">
			<header>				
				<h2>&nbsp;<i class="fa fa-adjust"></i> Employee List</h2>
			</header>
...
<div class="tab-content" >
	...
	<thead>
		<tr>
			...
			<th class="hasinput" style="width:16%"><input type="text" class="form-control" placeholder="Filter Status" /></th>
...
<?php
$empQ = "SELECT
	u_user.u_id, u_Status, `u_FullName`, `u_Cell`, `u_Email`,  `u_Address`, `u_Remarks`,
    role_name
FROM `u_user`
INNER JOIN sys_role on sys_role.role_id=u_user.role_id
WHERE u_user.branch_id=$branch_id and branch_admin=0";
$empRes = mysqli_query($con,$empQ);
while($empRow = mysqli_fetch_assoc($empRes))
{
	if($empRow['u_Status']==1)
	{
		$status ="Active";
	}
	elseif($empRow['u_Status']==0)
	{
		$status='In-Active';
	} 
?>
				 	<tr id="row<?php echo $empRow['u_id'];?>">
				 		<td><?php echo $empRow['u_FullName'];?></td>
                        <td><?php echo $status;?></td>
                        <td><?php echo $empRow['u_Cell'];?></td>
                        <td><?php echo $empRow['u_Email'];?></td>
                        <td><?php echo $empRow['u_Address'];?></td>
                        <td><?php echo $empRow['u_Remarks'];?></td>				 		
                        
				 		<td>
				 			<a href="#" onclick="del(<?=$empRow['u_id'];?>)" class="btn btn-danger btn-xs">DELETE</a>
				 		</td>
				 	</tr>
