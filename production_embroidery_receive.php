...
if($delete_id) {
    ...
    if($purchase_ids) {
        ...
        // Pehle purchase detail delete karo
        mysqli_query($con, "DELETE FROM adm_purchase_detail WHERE p_id IN ($ids)");
        // Phir purchase main record delete karo
    ...
    <table id="row-items-table" class="custom-table" style="width:100%; min-width:1000px">
        ...
        if($edit_id && !empty($details)) {
            foreach($details as $i => $row) {
                ...
                $item_name = isset($items[$row['item_id']]) ? $items[$row['item_id']] : '';
        ?>
            <tr>
                <td>
                ...
                <td>
                    <input type="text" class="form-control" name="rows[<?php echo $i; ?>][item_name]" value="<?php echo htmlspecialchars($item_name); ?>" readonly>
                    <input type="hidden" name="rows[<?php echo $i; ?>][item_id]" value="<?php echo $row['item_id']; ?>">
                </td>
                <td>
        ...
        <tr>
            ...
            <td>
                <input type="text" class="form-control" name="rows[0][item_name]" value="" readonly>
                <input type="hidden" name="rows[0][item_id]" value="">
            </td>
            <td>
...
$(document).ready(function() {
    ...
    $('#add-row-btn').on('click', function(){
        var newRow = `<tr>
            ...
            <td>
                <input type="text" class="form-control" name="rows[`+rowIdx+`][item_name]" value="" readonly>
                <input type="hidden" name="rows[`+rowIdx+`][item_id]" value="">
            </td>
            <td>
    ...
    console.log('updateIssuedFields:', lot_no, unit_id);
    if(lot_no && unit_id) {
        $.ajax({
            ...
            success: function(res) {
                ...
                // Auto set item name and id if returned
                if(res.item_id && res.item_name){
                    tr.find('input[name$="[item_id]"]').val(res.item_id);
                    tr.find('input[name$="[item_name]"]').val(res.item_name);
                }
            },
            error: function(xhr, status, error) {
                ...
                tr.find('input[name$="[item_id]"]').val('');
                tr.find('input[name$="[item_name]"]').val('');
            }
        });
    } else {
        // Only clear fields if lot or unit is missing
        tr.find('.issued-suits-field').val('');
        tr.find('.issued-mtrs-field').val('');
        tr.find('input[name$="[item_id]"]').val('');
        tr.find('input[name$="[item_name]"]').val('');
    ...
    $(document).on('change', '.lot-select', function(){
        console.log('Lot changed', $(this).val());
        var tr = $(this).closest('tr');
